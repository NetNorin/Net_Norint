...
def affine_to_list(affine_transform):
    return [affine_transform.a, affine_transform.b, affine_transform.c,
            affine_transform.d, affine_transform.e, affine_transform.f]


def preprocess_metadata(meta):
    # Select relevant metadata and convert to a simple dictionary
    processed_meta = {
        'width': meta['width'],
        'height': meta['height'],
        'crs': str(meta['crs']),  # Convert CRS to string
        'transform': affine_to_list(meta['transform'])  # Convert Affine transform to list using the function
    }
...
def read_image(path, normalize=True, value_discard=True):
    ...
    return np.transpose(img, (1, 2, 0)), preprocess_metadata(meta)
...
class ChangeDetectionDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_1, meta = read_image(myPath , self.bands, value_discard=self.value_discard)    # Image -> np.array, type: unit8
        myPath = os.path.join(path , 'after',  name)
        img_2, meta = read_image(myPath , self.bands, value_discard=self.value_discard)    # Image -> np.array, type: unit8
